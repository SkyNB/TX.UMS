var Validate = {
	...
	vdataTrues: function(value, config) {//返回正确 true false 对象
		var rs = {};
		for(var f in config) {
			if(typeof(config[f]) == "string") { config[f] = { msg: config[f] } }
			if(typeof this[f] == "function") { 
					rs[f] = (typeof config[f] == "object"?this[f](value, config[f].param):undefined); 
			} else if(typeof config[f] == "function") {
					rs[f] = config[f](value);
			} else{
				rs[f] = undefined;
			}
	...
	vobj: function(obj, configs) {//返回正确错误对象 提示 与结果
		var tmpr = { msg: "", result: true };
		for(var t in configs) {
			tmpr = this.vdata(obj[t], configs[t]);
			if(!tmpr.result) {
				return tmpr;
			}
		}
	...
	vobjTrues: function(obj, configs) {//返回正确 true false 对象数组
		var rss = [];
		for(var t in configs){
			rss.push(this.vdataright(obj[t],configs[t]))
		}
		return rss;
	},
}