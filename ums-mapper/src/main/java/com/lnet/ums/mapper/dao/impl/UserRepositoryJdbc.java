...
import static com.lnet.ums.contract.Utils.CacheConstants.USER_CACHE;
...
public class UserRepositoryJdbc implements UserRepository {
    ...
    @Override
    public OauthUser findByGuid(String guid) {
        final String sql = " select * from user_ where  guid = ? ";
        final List<OauthUser> list = this.jdbcTemplate.query(sql, new Object[]{guid}, userRowMapper);
    ...
    @Override
    public void saveUser(final OauthUser user) {
    ...
    public void updateUser(final OauthUser user) {
    ...
    public OauthUser findByUsername(String username){
        // TODO: 2017/2/11 需要重写
        final String sql = " select * from erp_user where userName = ? and archived = 0";

        final List<OauthUser> list = this.jdbcTemplate.query(sql, new Object[]{username}, userRowMapper);

        OauthUser user = null;
        if (!list.isEmpty()) {
            user = list.get(0);
            user.privileges().addAll(findPrivileges(String.valueOf(user.id())));
        }
    ...
    public List<OauthUser> findUsersByUsername(String username) {
        ...
        final List<OauthUser> list = this.jdbcTemplate.query(sql, params, userRowMapper);
        for (OauthUser user : list) {
            //user.privileges().addAll(findPrivileges(user.id()));
        }
        return list;
    }
